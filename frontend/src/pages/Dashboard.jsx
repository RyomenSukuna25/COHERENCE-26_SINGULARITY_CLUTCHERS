    ...
    <div style={{ minHeight:'100vh', background:'#080b11', color:'#e2e8f0', display:'flex', fontFamily:'JetBrains Mono, monospace' }}>
      ...
      <div style={{
        ...
        <div style={{ padding:'20px 16px', borderBottom:'1px solid rgba(255,255,255,0.05)',
          ...
          <button onClick={() => setSidebarCollapsed(c=>!c)} style={{
            background:'none', border:'none', color:'#374151', cursor:'pointer', fontSize:14, flexShrink:0, padding:4
          ...
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            ...
            {!sidebarCollapsed && (
              <div style={{ flex:1, overflow:'hidden' }}>
                <p style={{ color:'#e2e8f0', fontSize:11, fontWeight:600, whiteSpace:'nowrap' }}>
      ...
      <div style={{ marginLeft:sideW, flex:1, display:'flex', flexDirection:'column', transition:'margin-left 0.2s ease' }}>
        ...
        <div style={{ padding:'24px 28px', flex:1, overflowY:'auto' }} className='page-anim' key={page}>
                          ...
                          <td style={{ padding:'13px 16px' }}>
                            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                              ...
                              <div style={{ flex:1, height:3, background:'rgba(255,255,255,0.04)', borderRadius:2, minWidth:40 }}>
                                <div style={{ width:`${lead.score}%`, height:'100%', background:sc2, borderRadius:2, opacity:0.8 }}/>
                              </div>
                            </div>
                          </td>
                          <td style={{ padding:'13px 16px' }}>
          ...
          {/* UPLOAD */}
          {page==='upload' && <LeadUpload onLeadsLoaded={l => { if(l.length) setLeads(l); setPage('leads') }}/>}
            ...
            <div>
              <div style={{ marginBottom:20 }}>
                ...
                <p style={{ color:'#374151', fontSize:11, marginTop:2 }}>
                  Select a lead below → open their panel → Email tab → Generate → Mirror Check
                </p>
              </div>
          ...
          {/* WORKFLOW */}
          {page==='workflow' && <WorkflowPage leads={leads}/>}
...
function SequencePage({ leads }) {
  ...
  const SEQUENCES = [
    { id:1, name:'SaaS CTO Sequence', status:'active', leads:18, replied:4,
      steps:[
        { day:0,  type:'email',    persona:'dev',   subject:'Engineering outreach rate', action:'Send initial email via Dev persona' },
        { day:3,  type:'check',    persona:null,    subject:'Signal check', action:'If opened proceed. If cold LinkedIn fallback' },
        { day:5,  type:'email',    persona:'arjun', subject:'Follow-up: specific offer', action:'Send follow-up via Arjun persona' },
        { day:8,  type:'linkedin', persona:'dev',   subject:'LinkedIn connection', action:'Send LinkedIn message if email ignored' },
        { day:14, type:'email',    persona:'priya', subject:'Last touch', action:'Final email via Priya — warm, low pressure' },
      ]},
    ...
    { id:3, name:'CEO Outbound Sequence', status:'paused', leads:20, replied:5,
      steps:[
        { day:0,  type:'email',    persona:'arjun', subject:'Revenue outreach — direct', action:'Send initial email via Arjun' },
      ...
      <div style={{ display:'grid', gridTemplateColumns:'280px 1fr', gap:16 }}>
          ...
          {SEQUENCES.map(seq => (
            <div key={seq.id} onClick={() => setActiveSeq(seq)} style={{
              ...
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:8 }}>
                ...
                <span style={{ fontSize:9, padding:'2px 8px', borderRadius:20, letterSpacing:1,
              ...
              <div style={{ display:'flex', gap:16 }}>
                {[['Leads',seq.leads,'#e2e8f0'],['Replied',seq.replied,'#22c55e'],['Rate',Math.round(seq.replied/seq.leads*100)+'%','#a855f7']].map(([k,v,c])=>(
                  <div key={k}><p style={{ color:'#6b7280', fontSize:10 }}>{k}</p><p style={{ color:c, fontSize:14, fontWeight:600 }}>{v}</p></div>
                ))}
              </div>
            </div>
          ))}
        ...
        <div style={{ background:'rgba(255,255,255,0.025)', border:'1px solid rgba(255,255,255,0.05)', borderRadius:12, padding:24 }}>
          {!activeSeq ? (
            ...
            <div>
              <p style={{ color:'#e2e8f0', fontSize:14, fontWeight:600, fontFamily:'Inter,sans-serif', marginBottom:24 }}>{activeSeq.name}</p>
              <div style={{ position:'relative' }}>
                <div style={{ position:'absolute', left:18, top:20, bottom:20, width:1, background:'rgba(255,255,255,0.05)' }}/>
                {activeSeq.steps.map((step,i) => (
                  <div key={i} style={{ display:'flex', gap:16, marginBottom:20, position:'relative' }}>
                    <div style={{ width:36, height:36, borderRadius:'50%', flexShrink:0,
                      background:`${typeColors[step.type]}15`, border:`1px solid ${typeColors[step.type]}33`,
                      display:'flex', alignItems:'center', justifyContent:'center',
                      fontSize:11, fontWeight:700, color:typeColors[step.type], zIndex:1 }}>
                      {typeIcons[step.type]}
                    </div>
                    <div style={{ flex:1, background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.04)', borderRadius:10, padding:'12px 16px' }}>
                      ...
                      {step.persona && (
                        <span style={{ display:'inline-block', marginTop:6, fontSize:9, padding:'2px 8px', borderRadius:20,
