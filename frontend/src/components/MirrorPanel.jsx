...
export default function MirrorPanel({ lead, message, onApproved }) {
  ...
  const runMirror = async () => {
    ...
    try {
      ...
      setResult({ reaction: 'Could not simulate — backend error.', score: 45, objection: 'Unknown', suggested_fix: 'Check backend connection' })
    }
    setLoading(false)
  }
  ...
  const applyFix = async () => {
    ...
    try {
      ...
      // backend returns fixed_message OR message — handle both
      const improved_msg = fixRes.data.fixed_message || fixRes.data.message || message
      setImproved(improved_msg)
      setNewScore(fixRes.data.new_score || 72)
      onApproved && onApproved(improved_msg)
    } catch(e) {
      setImproved('Fix failed — try regenerating manually.')
    }
    setFixing(false)
  }
  ...
  return (
    <div className='bg-gray-900 rounded-xl p-5 border border-purple-800'>
      ...
      <div className='bg-gray-800 rounded-lg p-3 mb-4'>
        ...
        <p className='text-gray-300 text-sm line-clamp-3'>{message || 'No message yet — generate one first'}</p>
      </div>

      {!result && (
        <button onClick={runMirror} disabled={loading || !message}
        ...
        <div className='space-y-3'>
          <div className={`rounded-xl p-4 border ${scoreBg(result.score)}`}>
            <div className='flex items-center gap-4'>
              ...
              <div>
                ...
                <p className='text-gray-400 text-xs mt-1'>{result.verdict}</p>
              </div>
            </div>
          </div>
          ...
          <div className='grid grid-cols-2 gap-2'>
            ...
            <div className='bg-yellow-900/20 border border-yellow-900 rounded-lg p-3'>
              <p className='text-yellow-400 text-xs font-bold mb-1'>⚡ Suggested Fix</p>
              <p className='text-gray-300 text-xs'>{result.suggested_fix || '—'}</p>
            </div>
          </div>

          {/* Re-simulate button always available */}
          <button onClick={runMirror} disabled={loading}
            className='w-full bg-gray-800 hover:bg-gray-700 disabled:opacity-50 py-2 rounded-xl text-sm text-gray-400 transition-all'>
            ↺ Re-simulate
          </button>
            ...
            <div className='space-y-2'>
              <div className='bg-green-900/20 border border-green-700 rounded-xl p-4'>
                <div className='flex items-center justify-between mb-2'>
                  ...
                  {newScore && <span className={`text-lg font-bold ${scoreColor(newScore)}`}>{newScore}/100</span>}
                </div>
                <p className='text-gray-300 text-sm whitespace-pre-line'>{improved}</p>
              </div>
